...
# 01｜Primary Canon read order

1. `CURRENT.md`
2. `CANON-PRECEDENCE-V4.md`
3. `STORY-CAUSAL-CANON-PATCH-V1.md`
4. `SPATIAL-ENVIRONMENT-CANON-PATCH-V2.1-ASTEROID-CORE.md`
5. `SPATIAL-ENVIRONMENT-CANON-PATCH-V2.2-302-ORIGIN-LAYER.md` when 302 origin / asteroid relation is relevant
6. `SPATIAL-ENVIRONMENT-CANON-V2.md`
7. `EP01-EP10-SPATIAL-SCENE-MAP-V2.md`
8. `CHONGQING-TIME-MASTER-CANON-V4.md`
9. `WORLD-ARCHITECTURE-CANON-V4.md`
10. `CHARACTER-RELATIONSHIP-BIBLE-V4.md`
11. relevant highest active episode snapshot / approved outline / screenplay
12. current consistency reports only within their stated QA scope
...
- **Spatial asteroid-core patch:** `SPATIAL-ENVIRONMENT-CANON-PATCH-V2.1-ASTEROID-CORE.md`
- **302 origin / asteroid relationship patch:** `SPATIAL-ENVIRONMENT-CANON-PATCH-V2.2-302-ORIGIN-LAYER.md`
- **Spatial / environment baseline:** `SPATIAL-ENVIRONMENT-CANON-V2.md`
...
For 302 origin / asteroid-mining conflicts, V2.2 has higher priority than the narrower older 302 wording in Spatial V2 / Scene Map V2.
...
| EP05 | `Pilot-EP05-LATEST-CANON-SNAPSHOT-V5.md` |
| EP06 | `Pilot-EP06-LATEST-CANON-SNAPSHOT-V4.md` |
| EP07 | `Pilot-EP07-LATEST-CANON-SNAPSHOT-V4.md` |
| EP08 | `Pilot-EP08-LATEST-CANON-SNAPSHOT-V3.md` |
...
- **Active scene-generation workflow:** `SCENE-PROMPT-TEMPLATE-V3.md`
- **Legacy scene-generation provenance:** `SCENE-PROMPT-TEMPLATE-V2.md`
- **Detailed legacy anchor registry:** `VISUAL-ANCHOR-INDEX-V2.md`
- **Current anchor overlay:** `VISUAL-ANCHOR-INDEX-V4.md`
- **Active style router:** `VISUAL-STYLE-ROUTING-INDEX-V2.md`
- **Current teaser:** `TRAILER-TEASER-V3.md`

## Scene Prompt V3 — active atmosphere-first method

Status:

> **ACTIVE / DEFAULT FOR NEW MAJOR ENVIRONMENT + SCENE ATMOSPHERE IMAGES**
...
For `核心空间 / 场景气氛图 / 环境母图 / 大场景`, default shot mode is:

> **2.35:1 wide atmosphere master / establishing environment**

Core methodology:

> **Canon → shot mode → dominant action geography → scene identity → parent space → narrative affordances → composition → Oshii V1 → Seedance structural compression → generation → QC.**

Core rules:

- episode core space means the geography carrying most dominant physical action/state change, not simply the most prominent named node;
- scene identity must survive removal of signage;
- environment masters must establish the whole spatial system before local equipment;
- Seedance optimization = structural compression, not indiscriminate simplification;
- **删噪声，不删身份；合并结构，不削平空间。**
...
> **APPROVED / DEFAULT FOR NEW ENVIRONMENT + SETTING ART + SEEDANCE ENVIRONMENT SOURCES**

Core rule:

> **剧本 / Spatial Canon 决定“画什么”；Scene Prompt V3 决定“用什么场景级别和构图建立它”；押井守动画风格 V1 决定“怎么画”。**
...
- major atmosphere masters should not depend on signage for location identity.
...
> **PRESERVED AS VISUAL-DEVELOPMENT PROVENANCE / NOT CURRENT DEFAULT PRODUCTION NAME**
...
- **Earlier Seedance macro B master doc:** `docs/visual/2026-09-02-SECOND-NEW-CHONGQING-SEEDANCE-MASTER-B.md`
- **Earlier Seedance 2.5 exterior prompt B:** `docs/visual/2026-09-02-SECOND-NEW-CHONGQING-SEEDANCE-VIDEO-PROMPT-B.md`

Earlier B remains macro-geometry / continuity provenance. The approved Oshii Animation Style V1 master remains the current exterior rendering/style reference.

---

# 04｜Mandatory workflow for future setting images

Unless the user explicitly chooses another style, every new major setting image follows:

1. read current explicit user request;
2. read `CURRENT.md` / current precedence;
3. read relevant Spatial V2.1 / V2.2 / V2 authority;
4. read relevant episode snapshot / outline / screenplay;
5. route through `SCENE-PROMPT-TEMPLATE-V3.md`;
6. determine shot mode and dominant action geography;
7. identify parent city/spatial asset and valid geometry anchor;
8. identify 2–4 large semantic anchors and narrative affordances;
9. choose composition family intentionally;
10. apply **OSHII ANIMATION STYLE V1**;
11. apply scene-specific light/time state;
12. perform Seedance structural compression;
13. generate one production reference image;
14. run thumbnail / relabel / story-action / Seedance-motion QC.
...
- strong large light-shadow relationships;
- simplified illustrated mechanical linework;
- 2–3 hard cel-shadow tiers;
- sparse muted rust-red accents;
- selected red-white functional identification / repair bands when appropriate;
...
Do not add visual noise merely to increase realism.

---

# 06｜Spatial V2.1 + V2.2 + V2 hard locks
...
- central asteroid is captured/stabilized/engineered into the city and is distinct from the T0 impactor;
- central asteroid is not the normal 1G gravity source;
- ring topology does not generate gravity;
- design scale direction remains ≈ `1 km` main-ring radius / `200 m` major interior clear-height unless later explicitly changed;
- central geometry = asteroid core + engineered dock/reinforcement structure + radial structural/service links + bounded work/transport volumes;
- district modules are normal lived city spaces;
- `嘉陵江` = **finite continuous open urban/public corridor inside the ring**, not the torus center hole, not the asteroid itself, and not an infinite canyon;
...
- 302 = **distributed legacy industrial–residential–transfer–compatibility complex**, with outer lived/service layers and an asteroid-core mining origin layer;
- not all 302 is inside the asteroid and not all 302 is on the outer ring;
- EP01 remains entirely on the outer 302 public/service/space-facing route and does not detour through the mine;
- EP01 loses only a small 302 outer attached maintenance/interface segment, not 302 itself;
- asteroid mining origin layer mainly supports EP09 spatial evidence and EP10 historical retirement echo;
- EP10 bulk 302 migration largely predates the episode; small lifeline remains the last active legacy connection;
- city contraction retires modules/connections, not the permanent ring backbone or central asteroid core.

A possible later EP10 macro Legacy Sector separation remains provisional until explicitly promoted into Canon.
...
# 08｜Visual anchor use

`RING-ORIENT-01` and previous approved exterior work remain geometry/landmark provenance for:
...
Current preferred exterior visual/style master:

> `/重庆时间/visual/style-masters/SECOND_NEW_CHONGQING_OSHII_ANIMATION_STYLE_MASTER_V1.png`

For newly generated setting art, use Scene Prompt V3 + Oshii Animation Style V1 unless explicitly overridden.
...
Do not let style or composition inflate object scale.

---

# 09｜Visual-development priority

Current environment-master direction should proceed through a small number of reusable atmosphere spaces rather than ten unrelated spectacle locations.

Priority families include:

1. Second New Chongqing exterior master — already established;
2. 302 outer service / Gravity Service Spine atmosphere family for EP01;
3. finite Jialing wide atmosphere with readable bridge hierarchy;
4. EP02 Level-1 Trunk Bridge;
5. same small Familiar Lifeline + both bridgeheads;
6. 302 lived/public + old cinema family;
7. EP06/07 shared-infrastructure family;
8. 302 asteroid mining origin layer for EP09/EP10;
9. retained-city finale state when required.

Reuse locations and change their state; do not manufacture escalation by adding endless new megastructures.

---

# 10｜Consistency / verification

Spatial baseline verification remains valid only for its original scope.

Current post-asteroid-core / 302-origin status:

> **ASTEROID CORE V2.1 + 302 ORIGIN V2.2 ACTIVE.**

Visual routing requires separate visual QC and does not change Canon verification status.
...
Important visual decision logs include:
...
- `decision-logs/2026-09-03-0035-OSHII-ANIMATION-STYLE-V1-RENAME.md`
- `decision-logs/2026-09-03-1408-SCENE-IDENTITY-FIRST-METHOD.md`
- `decision-logs/2026-09-03-1510-ATMOSPHERE-FIRST-SCENE-V3.md`
...
> **SCRIPT / SPATIAL CANON → SCENE PROMPT V3 → VALID GEOMETRY ANCHOR → OSHII ANIMATION STYLE V1 → SEEDANCE STRUCTURAL COMPRESSION**

unless the user explicitly routes the scene to another style or shot mode.

For major structural state changes:

> **lock story geometry first → create/approve target-state atmosphere/keyframe → then animate.**
